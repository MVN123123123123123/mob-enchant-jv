        ...
        for (const enchant of enchantList) {
            switch (enchant.id) {
                case "infinity": {
                    ...
                    if (!isBroken) {
                        entity.addEffect("resistance", 20000000, { amplifier: 255, showParticles: true });
                    }
                    break;
                }
...
world.afterEvents.entityHurt.subscribe((event) => {
    ...
    // --- MENDING (Attacker): Heal whenever dealing damage ---
    if (attacker && isEntityValid(attacker) && isMob(attacker)) {
        const mending = getEnchant(attacker, "mending");
        if (mending) {
            ...
            if (healthComp) {
                ...
                const healAmount = mending.level * 2;
...
world.afterEvents.entityDie.subscribe((event) => {
    ...
    try {
        ...
        world.sendMessage(
            ...
            `§8[§e${enchantNames}§8] ` +
            `§6+${bonusXP} bonus XP §8| §a${totalItemsToDrop} bonus drops`
        );
...
world.afterEvents.effectAdd.subscribe((event) => {
    ...
    try {
        const isBroken = entity.getDynamicProperty("mob_enchant:infinity_broken") === true;
        if (!isBroken) {
            // Ignore if the effect being added is Resistance (our own shield)
            if (event.effect && event.effect.typeId === "minecraft:resistance") return;

            entity.setDynamicProperty("mob_enchant:infinity_broken", true);
            entity.removeEffect("resistance"); // Drop the shield
...
// --- Debug chat command (chatSend is beta-only, so wrap in try-catch) ---
try {
    world.beforeEvents.chatSend.subscribe((event) => {
        if (event.message === "!debug") {
            event.cancel = true;
            system.run(() => {
                world.sendMessage("DEBUG: The script is alive and running!");
            });
        }
    });
} catch {
    // chatSend is not available in stable @minecraft/server 1.11.0
}
...
system.afterEvents.scriptEventReceive.subscribe((event) => {
    // Debug print
    try {
        world.sendMessage(`DEBUG: Received scriptevent id=${event.id}`);
        if (event.sourceEntity) {
            world.sendMessage(`DEBUG: sourceEntity typeId=${event.sourceEntity.typeId}`);
        } else {
            world.sendMessage(`DEBUG: sourceEntity is undefined`);
        }
    } catch {}
    ...
    if (!player || player.typeId !== "minecraft:player") {
        try { world.sendMessage("DEBUG: Player check failed."); } catch {}
        return;
    }

    const action = event.id.replace("mobenchant:", "");
    const message = event.message ? event.message.trim() : "";
    ...
    if (action === "help") {
        tellPlayer(player, `§d§l✦ Mob Enchant Commands §r`);
        tellPlayer(player, `§8- §e/scriptevent mobenchant:enchant§7: Random enchant nearest mob`);
        tellPlayer(player, `§8- §e/scriptevent mobenchant:enchant sharpness 5§7: Specific enchant`);
        tellPlayer(player, `§8- §e/scriptevent mobenchant:random 3§7: N random enchants`);
        tellPlayer(player, `§8- §e/scriptevent mobenchant:clear§7: Remove all enchants`);
        tellPlayer(player, `§8- §e/scriptevent mobenchant:list§7: Show all available enchants`);
        tellPlayer(player, `§8- §e/scriptevent mobenchant:info§7: Inspect nearest mob`);
        return;
    }
}, { namespaces: ["mobenchant"] });
